sparql(
    "SELECT DISTINCT ?carrera ?sede ?universidad ?region ?vacantes WHERE { GRAPH <http://verdata.cl/graphs/ofertaacademica> {?c <http://verdata.cl/graphs/ofertaacademica/carrera/nombreCarrera> ?carrera." +
...
"?c <http://verdata.cl/graphs/ofertaacademica/carrera/area> 'Educación'@es." +
"?c <http://verdata.cl/graphs/ofertaacademica/carrera/vacantes> ?vacantes.}} ORDER BY DESC(?vacantes)",
        ...
        $.each(data.results.bindings, function(index, value) {
            $('#data').append(
                $('<tr>')
					...
					.append($('<td>').text(value.vacantes.value))
            );
        });
    });